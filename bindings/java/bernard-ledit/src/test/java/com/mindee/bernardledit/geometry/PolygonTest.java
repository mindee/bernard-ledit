...
public class PolygonTest {
  ...
  public void testInvalidCoordinateArrayLengthThrowsException() {
    ...
    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, () -> new Polygon(invalidCoords));
  ...
  void testEqualsAndHashCodeContract() {
    ...
    Polygon poly1 = new Polygon(Arrays.asList(p1, p2));
    Polygon poly2 = new Polygon(Arrays.asList(p1, p2));
    assertEquals(poly1, poly2, "Polygons with same points must be equal");
    assertEquals(poly1.hashCode(), poly2.hashCode(), "Equal objects must have the same hashCode");
    ...
    assertTrue(
        set.contains(poly2),
        "HashSet should find poly2 because it equals poly1 and has the same hash");

    assertEquals(1, set.size(), "HashSet should not allow duplicate Polygons");
  }
}
